...
# V7 = Pendulum Color Version 2 Deep Savers
SRC = r"c:/Users/roy/CodeBuddy/Digital-MonsterVpet/Digital-MonsterVpet/sprites/V7/LCD Handhelds - Digimon Pendulum Color - Digimon - Version 2 Deep Savers.png"
OUT_DIR = r"c:/Users/roy/CodeBuddy/Digital-MonsterVpet/Digital-MonsterVpet/sprites/V7"
os.makedirs(OUT_DIR, exist_ok=True)

img = Image.open(SRC).convert("RGBA")
pix = img.load()
w, h = img.size
print(f"V7 原圖尺寸: {w} x {h}")
...
print(f"行起點: {ROW_STARTS[:5]}...{ROW_STARTS[-3:] if len(ROW_STARTS)>5 else ''}")
...
if len(all_tiles) > 0 and len(all_tiles[0]) > 0:
    ...
    if has_content:
        egg_tile.save(os.path.join(OUT_DIR, "V7_egg.png"))
        print("[V7_egg.png] 已保存")
...
for r in range(num_rows_out):
    ...
    fname = os.path.join(OUT_DIR, f"V7_{r:02d}.png")
    ri.save(fname)
    print(f"[V7_{r:02d}.png] {num_cols} 幀已保存")
