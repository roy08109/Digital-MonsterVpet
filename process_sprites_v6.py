...
# V6 = Pendulum Color Nature Spirits
SRC = r"c:/Users/roy/CodeBuddy/Digital-MonsterVpet/sprites/V6/V6.png"
OUT_DIR = r"c:/Users/roy/CodeBuddy/Digital-MonsterVpet/sprites/V6"
os.makedirs(OUT_DIR, exist_ok=True)

img = Image.open(SRC).convert("RGBA")
pix = img.load()
w, h = img.size
print(f"V6 原圖尺寸: {w} x {h}")
...
if len(all_tiles) > 0 and len(all_tiles[0]) > 0:
    ...
    if has_content:
        egg_tile.save(os.path.join(OUT_DIR, "V6_egg.png"))
        print("[V6_egg.png] 已保存")

# 5) 每行存一張 V6_XX.png（V6 實際檢測 32 行全部有內容）
...
for r in range(num_rows_out):
    ...
    fname = os.path.join(OUT_DIR, f"V6_{r:02d}.png")
    ri.save(fname)
    print(f"[V6_{r:02d}.png] {num_cols} 幀已保存")
