...
SRC = r"c:/Users/roy/CodeBuddy/Digital MonsterVpet/Digital-MonsterVpet/sprites/V5/LCD Handhelds - Digital Monster Color - Digimon - Version 5.png"
OUT_DIR = r"c:/Users/roy/CodeBuddy/Digital MonsterVpet/Digital-MonsterVpet/sprites/V5"
os.makedirs(OUT_DIR, exist_ok=True)

img = Image.open(SRC).convert("RGBA")
pix = img.load()
w, h = img.size
print(f"V5 原圖尺寸: {w} x {h}")
...
if len(all_tiles) > 0 and len(all_tiles[0]) > 0:
    ...
    if has_content:
        egg_tile.save(os.path.join(OUT_DIR, "V5_egg.png"))
        print("[V5_egg.png] 已保存")

# 5) 每行存一張 V5_XX.png（V5 共 19 行）
...
for r in range(num_rows_out):
    ...
    fname = os.path.join(OUT_DIR, f"V5_{r:02d}.png")
    ri.save(fname)
    print(f"[V5_{r:02d}.png] {num_cols} 幀已保存")
