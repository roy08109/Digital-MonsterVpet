...
SRC = r"c:/Users/roy/CodeBuddy/Digital MonsterVpet/sprites/V2/LCD Handhelds - Digital Monster Color - Digimon - Version 2.png"
OUT_DIR = r"c:/Users/roy/CodeBuddy/Digital MonsterVpet/sprites/V2"
os.makedirs(OUT_DIR, exist_ok=True)

img = Image.open(SRC).convert("RGBA")
pix = img.load()
w, h = img.size
print(f"V2 原圖尺寸: {w} x {h}")
...
def find_largest_grid(col_starts, row_starts):
    ...
    for i, cs in enumerate(col_starts):
        ...
        for j in range(i+1, len(col_starts)):
            if col_starts[j] - group[-1] == 17:  # 16 + 1px gap
...
if len(all_tiles) > 0 and len(all_tiles[0]) > 0:
    ...
    if has_content:
        egg_tile.save(os.path.join(OUT_DIR, "egg.png"))
        print("[egg.png] 已保存")

# 5) 每行存一張 V2_XX.png（與 V1 命名對齊）
num_rows_out = min(num_rows, 18)
GAP = 1
for r in range(num_rows_out):
    ...
    fname = os.path.join(OUT_DIR, f"V2_{r:02d}.png")
    ri.save(fname)
    print(f"[V2_{r:02d}.png] {num_cols} 幀已保存")
